...
class controller:
    # initialize map
    def __init__(self,col=5,row=5,objs=4,pool_size=10,init_pool=[],mode="3"):
        self.Map=None
        if(not (pool_size*objs<col*row and len(init_pool)<col*row)):
            self.Map=map.Map(col,row,objs,pool_size,init_pool,mode)
    #  start solving
    def start(self):
        if(self.Map==None):
...
if __name__ == "__main__":
   c=controller(10,10,4,100,[],"random")
   c.start()
